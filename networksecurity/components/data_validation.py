...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe:pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Expected numerical columns from schema: {expected_numerical_columns}")
            actual_columns = dataframe.columns.tolist()

            for col in expected_numerical_columns:
                if col not in actual_columns:
                    logging.info(f"Missing numerical columns: {col}")
                    return False
            return True
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold = 0.05) -> bool:
        try:
            ...
            drift_report_file_part = self.data_validation_config.drift_report_file_path

            ## create directory
            dir_path = os.path.dirname(drift_report_file_part)
            os.makedirs(dir_path, exist_ok = True)     
            write_yaml_file(file_path = drift_report_file_part, content = report)
